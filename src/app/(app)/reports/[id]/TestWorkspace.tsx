...
function N({ value, onChange, disabled, placeholder }: { value: number | null | undefined; onChange: (v: number | null) => void; disabled?: boolean; placeholder?: string }) {
  return <input type="number" step="any" className={cell} disabled={disabled} placeholder={placeholder} value={value ?? ""} onChange={(e) => onChange(num(e.target.value))} />;
}
                ...
                <tbody>
                  {W.rows.map((r, i) => (
                    <tr key={i}>
                      ...
                      <td className="p-1"><N disabled={dis} value={r.indUp} onChange={(v) => upd("weighing", { ...W, rows: W.rows.map((x, j) => (j === i ? { ...x, indUp: v } : x)) })} /></td>
                      ...
                      <td className="p-1"><N disabled={dis} value={r.indDown} onChange={(v) => upd("weighing", { ...W, rows: W.rows.map((x, j) => (j === i ? { ...x, indDown: v } : x)) })} /></td>
              ...
              {R.series.map((s, si) => (
                <div key={si} className="mb-4 rounded-lg border border-slate-100 p-3">
                  ...
                  <div className="grid grid-cols-5 gap-2 md:grid-cols-10">
                    {s.readings.map((v, ri) => <N key={ri} disabled={dis} placeholder={`#${ri + 1}`} value={v} onChange={(nv) => upd("repeatability", { ...R, series: R.series.map((x, j) => (j === si ? { ...x, readings: x.readings.map((y, k) => (k === ri ? nv : y)) } : x)) })} />)}
                  </div>
              ...
              <table className="w-full"><thead><tr><th className={th}>Position</th><th className={th}>Indication</th>{W.method === "changeover" && <th className={th}>ΔL</th>}<th className={th}></th></tr></thead>
                <tbody>{E.positions.map((p, i) => (
                  <tr key={i}>
                    ...
                    <td className="p-1"><N disabled={dis} value={p.indication} onChange={(v) => upd("eccentricity", { ...E, positions: E.positions.map((x, j) => (j === i ? { ...x, indication: v } : x)) })} /></td>
              ...
              <table className="w-full"><thead><tr><th className={th}>Net load ({u})</th><th className={th}>Indication (net)</th>{W.method === "changeover" && <th className={th}>ΔL</th>}<th className={th}>MPE</th><th className={th}></th></tr></thead>
                <tbody>{T.rows.map((r, i) => (
                  <tr key={i}>
                    ...
                    <td className="p-1"><N disabled={dis} value={r.indication} onChange={(v) => upd("tare", { ...T, rows: T.rows.map((x, j) => (j === i ? { ...x, indication: v } : x)) })} /></td>
              ...
              <table className="w-full"><thead><tr><th className={th}>Load ({u})</th><th className={th}>Indication I₁</th><th className={th}>Indication I₂ (after +1.4 d)</th><th className={th}></th></tr></thead>
                <tbody>{D.rows.map((r, i) => (
                  <tr key={i}>
                    ...
                    <td className="p-1"><N disabled={dis} value={r.indication} onChange={(v) => upd("discrimination", { ...D, rows: D.rows.map((x, j) => (j === i ? { ...x, indication: v } : x)) })} /></td>
                    <td className="p-1"><N disabled={dis} value={r.indicationAfter} onChange={(v) => upd("discrimination", { ...D, rows: D.rows.map((x, j) => (j === i ? { ...x, indicationAfter: v } : x)) })} /></td>
              ...
              <table className="w-full"><thead><tr><th className={th}>Time after switch-on (min)</th><th className={th}>Zero indication</th><th className={th}>Indication at load</th><th className={th}></th></tr></thead>
                <tbody>{WU.rows.map((r, i) => (
                  <tr key={i}>
                    ...
                    <td className="p-1"><N disabled={dis} value={r.indication} onChange={(v) => upd("warmup", { ...WU, rows: WU.rows.map((x, j) => (j === i ? { ...x, indication: v } : x)) })} /></td>
              ...
              <table className="w-full"><thead><tr><th className={th}>Temp (°C)</th><th className={th}>Load ({u})</th><th className={th}>Zero indication</th><th className={th}>Indication</th><th className={th}></th></tr></thead>
                <tbody>{TP.rows.map((r, i) => (
                  <tr key={i}>
                    ...
                    <td className="p-1"><N disabled={dis} value={r.indication} onChange={(v) => upd("temperature", { ...TP, rows: TP.rows.map((x, j) => (j === i ? { ...x, indication: v } : x)) })} /></td>
              ...
              <table className="w-full"><thead><tr><th className={th}>Voltage (V)</th><th className={th}>Load ({u})</th><th className={th}>Indication</th><th className={th}></th></tr></thead>
                <tbody>{V.rows.map((r, i) => (
                  <tr key={i}>
                    ...
                    <td className="p-1"><N disabled={dis} value={r.indication} onChange={(v) => upd("voltage", { ...V, rows: V.rows.map((x, j) => (j === i ? { ...x, indication: v } : x)) })} /></td>
              ...
              <table className="w-full"><thead><tr><th className={th}>Date</th><th className={th}>Indication</th><th className={th}></th></tr></thead>
                <tbody>{S.rows.map((r, i) => (
                  <tr key={i}>
                    ...
                    <td className="p-1"><N disabled={dis} value={r.indication} onChange={(v) => upd("spanStability", { ...S, rows: S.rows.map((x, j) => (j === i ? { ...x, indication: v } : x)) })} /></td>
              ...
              <table className="w-full"><thead><tr><th className={th}>Condition</th><th className={th}>Load ({u})</th><th className={th}>Indication</th><th className={th}></th></tr></thead>
                <tbody>{DH.rows.map((r, i) => (
                  <tr key={i}>
                    ...
                    <td className="p-1"><N disabled={dis} value={r.indication} onChange={(v) => upd("dampHeat", { ...DH, rows: DH.rows.map((x, j) => (j === i ? { ...x, indication: v } : x)) })} /></td>
